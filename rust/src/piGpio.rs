...
// }
impl<P> Pins<P> where P: gpio::GpioOut<Error = ()> {
...
pub fn testing() {
    let mut pinstruct = Pins{ 
        logging_pins :  (0..26).map(|pin_num| {
        DummyGpioOut::new(move |value| println!("pin {} set to {:?}", pin_num, value))
      }).collect()
    };
    ...
    let mut logging_pins: Vec<_> = (0..26).map(|pin_num| {
        DummyGpioOut::new(move |value| println!("pin {} set to {:?}", pin_num, value))
      }).collect();
